...
# Remembers last seen mileage per train to detect updates
_last_mileage: dict = {}

# Stores dismissed alert keys so they don't reappear after reload
_dismissed: set = set()
...
def get_milestone_alerts(trains):
    ...
    for train in trains:
        if train["status"] == "maintenance":
            continue  # already in maintenance, no alert needed
        ...
        for ms in MILESTONES:
            ...
            # If mileage changed since last seen, reset timestamp and clear dismissal
            if _last_mileage.get(train['id']) != mileage:
                _last_mileage[train['id']] = mileage
                if key in _alert_first_seen:
                    _alert_first_seen[key] = datetime.now().strftime("%H:%M")
                # Clear any dismissal for this train so updated alert re-appears
                for dk in list(_dismissed):
                    if dk.startswith(train['id']):
                        _dismissed.discard(dk)

            # Check if PM was already done for this next_due milestone
            # To mark as serviced, add next_due as a number to the train's "serviced_at" list
            # e.g. "serviced_at": [360000] means the 360,000 km PM has been completed
            serviced_at = train.get("serviced_at", [])
            if next_due in serviced_at:
                continue  # PM was done for this cycle, skip alert
        ...
        # If Full Overhaul (360K) is triggered, suppress all smaller milestones
        # No point flagging 2K/13K/40K/120K checks when an overhaul is already due
        has_overhaul = any(m["cycle"] == "360,000 km" for m in triggered)
        if has_overhaul:
            triggered = [m for m in triggered if m["cycle"] == "360,000 km"]
        ...
        if len(triggered) == 1:
            ...
            km_lines = " | ".join([
                f"{m['cycle']}: {m['km_remaining']:,} km away" if m["status"] == "upcoming"
                else f"{m['cycle']}: due now"
                for m in triggered
            ])
            message = f"{train['name']} ({train['id']}) — {km_lines}."

        # Build a stable key for this alert card to check against dismissed set
        dismiss_key = f"{train['id']}-{'-'.join([m['cycle'] for m in triggered])}"
        if dismiss_key in _dismissed:
            continue  # user dismissed this, skip it
...
async def dismiss_alert(payload: dict):
    key = payload.get("dismiss_key", "")
    if key:
        _dismissed.add(key)
    return {"ok": True}
