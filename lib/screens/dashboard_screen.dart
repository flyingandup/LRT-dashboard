...
class DashboardScreen extends StatelessWidget {
      ...
      body: StreamBuilder<List<Train>>(
        ...
        builder: (context, snapshot) {
          return Column(children: [
            AppHeader(isConnected: snapshot.hasData),
  ...
  Widget _buildBody(AsyncSnapshot<List<Train>> snapshot) {
    if (snapshot.hasError) {
      return Center(
        ...
        Text('Firebase connection error',
            style: GoogleFonts.barlow(color: AppColors.muted, fontSize: 15)),
        const SizedBox(height: 8),
        Text('${snapshot.error}',
            style: GoogleFonts.dmMono(color: AppColors.muted, fontSize: 12)),
      ]));
    }
    ...
    final activePct =
        trains.isNotEmpty ? (active / trains.length * 100).round() : 0;
    final maintPct =
        trains.isNotEmpty ? (maint / trains.length * 100).round() : 0;
        ...
        Row(children: [
          ...
          Expanded(
              child: KpiCard(
                  ...
                  value: '$maint',
                  sub: '$maintPct% of fleet',
                  accentColor: AppColors.maint)),
          const SizedBox(width: 16),
          Expanded(
              child: KpiCard(
            label: 'Active Alerts',
        ...
        SizedBox(
          ...
          child: Row(children: [
            ...
            Expanded(
              flex: bpFlex,
              child: _chartPanel(
                  child: MileageBarChart(
                      trains: bpTrains,
                      title: 'Bukit Panjang (${bpTrains.length})')),
            ),
            ...
            Expanded(
              ...
              child: _chartPanel(
                  child: MileageBarChart(
                      trains: pgTrains, title: 'Punggol (${pgTrains.length})')),
            ),
            const SizedBox(width: 12),
            ...
            SizedBox(
              ...
              child: _chartPanel(
                  child: MileageBarChart(
                      trains: skTrains,
                      title: 'Sengkang (${skTrains.length})')),
            ),
          ]),
        ),
        const SizedBox(height: 24),
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Row(children: [
                          _alertMini('Critical', counts['critical'] ?? 0,
                              AppColors.critical),
                          const SizedBox(width: 8),
                          _alertMini('Warning', counts['warning'] ?? 0,
                              AppColors.warning),
                          const SizedBox(width: 8),
                          _alertMini(
                              'Info', counts['info'] ?? 0, AppColors.info),
                        ]),
                      ),
                      ...
                      if (groupedAlerts.isEmpty)
                        Padding(
                          ...
                          child: Center(
                              child: Text('No alerts — all trains on track.',
                        ...
                        ConstrainedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (_, i) =>
                                GroupedAlertItem(alert: groupedAlerts[i]),
                          ),
                        ),
                    ]),
              ),
