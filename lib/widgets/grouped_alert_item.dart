...
class GroupedAlertItem extends StatelessWidget {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Milestone entries
                  ...alert.entries.map((e) => _buildEntry(context, e)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildEntry(BuildContext context, MilestoneEntry e) {
    final color = AppColors.severityColor(e.severity);
    final kmText =
        e.kmRemaining == 0 ? 'Due now' : 'in ${_fmt(e.kmRemaining)} km';
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () => _showClearConfirmation(context, e),
          ),
        ],
      ),
    );
  ...
  void _showClearConfirmation(BuildContext context, MilestoneEntry e) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          backgroundColor: AppColors.surface,
          title: Text('Resolve Maintenance Task',
              style: GoogleFonts.barlow(
                  fontWeight: FontWeight.w600, color: AppColors.textMain)),
          content: Text(
            'Confirming completion of ${e.label} for ${alert.trainId}. This action resets the distance threshold calculation using the current track mileage (${_fmt(alert.trainMileage)} km).',
            style: GoogleFonts.barlow(
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.severityColor(e.severity),
                elevation: 0,
              ),
              onPressed: () async {
                Navigator.of(dialogContext).pop();
                try {
                  await FirebaseService.clearMilestoneAlert(
                    alert.trainId,
                    e.cycle,
                    alert.trainMileage,
                  );
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                            'Successfully closed work alert entry: ${e.label}'),
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                            'Error clearing maintenance alert registry: $error'),
